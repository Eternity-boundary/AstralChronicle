...
namespace winrt::AstralChronicle::implementation
{
    ...
    void SessionsViewModel::Initialize(std::shared_ptr<::AstralChronicle::services::ISessionRepository> repo, std::shared_ptr<::AstralChronicle::design::IStringResourceService> strings, Microsoft::UI::Dispatching::DispatcherQueue const& dispatcher){m_repo=std::move(repo);m_strings=std::move(strings);if(!m_repo||!m_strings)throw std::invalid_argument("Sessions require repository and string services.");m_dispatcher=dispatcher;m_heading=m_strings->GetString(L"Sessions.Heading");m_summary=m_strings->GetString(L"Sessions.Summary");NewSession();Refresh();}
    ...
    winrt::fire_and_forget SessionsViewModel::LoadAsync(std::uint64_t v)
    {
        auto life = get_strong();
        try
        {
            auto const repo = m_repo;
            auto const dispatcher = m_dispatcher;
            if (!repo || !dispatcher) co_return;
            bool failed{};
            std::vector<::AstralChronicle::models::DiagnosticSession> all;
            co_await winrt::resume_background();
            try { all = repo->Load(); }
            catch (...) { failed = true; }
            co_await wil::resume_foreground(dispatcher);
            if (v != m_version) co_return;
            if (failed)
            {
                m_loading = false;
                SetStatus(m_strings->GetString(L"Sessions.SaveFailed.Text"), {}, Microsoft::UI::Xaml::Controls::InfoBarSeverity::Error);
                Raise(L"IsLoading");
                co_return;
            }
            Apply(std::move(all));
    ...
    void SessionsViewModel::Apply(std::vector<::AstralChronicle::models::DiagnosticSession> all)
    {
        auto selectedId = m_pendingSelectedId;
        if (selectedId.empty() && m_hasSelection && m_selectedIndex >= 0 && static_cast<std::size_t>(m_selectedIndex) < m_models.size())
            selectedId = m_models[static_cast<std::size_t>(m_selectedIndex)].Id;
        std::sort(all.begin(), all.end(), [](auto const& a, auto const& b)
        {
            return a.IsPinned != b.IsPinned ? a.IsPinned > b.IsPinned : a.Name < b.Name;
        });
        m_models = std::move(all);
        m_selectedIndex = -1;
        m_hasSelection = false;
        auto vals = winrt::single_threaded_observable_vector<winrt::hstring>();
        for (auto const& s : m_models)
        {
            vals.Append(winrt::hstring{ s.Name });
            if (!selectedId.empty() && s.Id == selectedId)
            {
                m_selectedIndex = static_cast<std::int32_t>(vals.Size() - 1);
                m_hasSelection = true;
                Fill(s);
            }
        }
        if (!selectedId.empty() && !m_hasSelection)
        {
            NewSession();
        }
        ...
        m_sessions = vals;
        m_loading = false;
        m_summary = m_strings->GetString(L"Sessions.SummaryCount.Text");
        m_hasStatus = false;
        Raise(L"Sessions"); Raise(L"Summary"); Raise(L"SelectedIndex"); Raise(L"HasSelection");
        RaiseEditor(); Raise(L"IsLoading"); Raise(L"HasStatusMessage");
    }
    void SessionsViewModel::SelectedIndex(std::int32_t index)
    {
        ...
        m_hasSelection = true;
        Fill(m_models[static_cast<std::size_t>(index)]);
        Raise(L"SelectedIndex"); Raise(L"HasSelection"); RaiseEditor();
    }
    ...
    void SessionsViewModel::Save(){if(!m_repo||!CanSave()){SetStatus(m_strings->GetString(L"Sessions.NameRequired.Text"),{},Microsoft::UI::Xaml::Controls::InfoBarSeverity::Warning);return;}auto s=Build();if(m_repo->Upsert(s)){m_pendingSelectedId=s.Id;SetStatus(m_strings->GetString(L"Sessions.Saved.Text"),{},Microsoft::UI::Xaml::Controls::InfoBarSeverity::Success);Refresh();}else SetStatus(m_strings->GetString(L"Sessions.SaveFailed.Text"),{},Microsoft::UI::Xaml::Controls::InfoBarSeverity::Error);}
    void SessionsViewModel::Delete(){if(!m_repo||!m_hasSelection)return;if(m_repo->Remove(m_models[static_cast<std::size_t>(m_selectedIndex)].Id)){NewSession();SetStatus(m_strings->GetString(L"Sessions.Deleted.Text"),{},Microsoft::UI::Xaml::Controls::InfoBarSeverity::Success);Refresh();}else SetStatus(m_strings->GetString(L"Sessions.SaveFailed.Text"),{},Microsoft::UI::Xaml::Controls::InfoBarSeverity::Error);}
    void SessionsViewModel::Duplicate(){if(!m_hasSelection)return;auto s=Build();s.Id=NewId()+L"-copy";s.Name+=L" (copy)";s.IsActive=false;if(m_repo&&m_repo->Upsert(s)){m_pendingSelectedId=s.Id;SetStatus(m_strings->GetString(L"Sessions.Duplicated.Text"),{},Microsoft::UI::Xaml::Controls::InfoBarSeverity::Success);Refresh();}else SetStatus(m_strings->GetString(L"Sessions.SaveFailed.Text"),{},Microsoft::UI::Xaml::Controls::InfoBarSeverity::Error);}
    void SessionsViewModel::Archive(){if(!m_hasSelection||!m_repo)return;auto s=Build();s.IsArchived=!s.IsArchived;s.IsPinned=false;if(m_repo->Upsert(s)){m_pendingSelectedId=s.Id;Refresh();}else SetStatus(m_strings->GetString(L"Sessions.SaveFailed.Text"),{},Microsoft::UI::Xaml::Controls::InfoBarSeverity::Error);}
    void SessionsViewModel::TogglePin(){if(!m_hasSelection||!m_repo)return;auto s=Build();s.IsPinned=!s.IsPinned;if(m_repo->Upsert(s)){m_pendingSelectedId=s.Id;Refresh();}else SetStatus(m_strings->GetString(L"Sessions.SaveFailed.Text"),{},Microsoft::UI::Xaml::Controls::InfoBarSeverity::Error);}
    void SessionsViewModel::Resume(){if(!m_hasSelection||!m_repo)return;auto s=Build();s.IsActive=true;s.LastResumedAt=Now();if(m_repo->Upsert(s)){m_pendingSelectedId=s.Id;SetStatus(m_strings->GetString(L"Sessions.Resumed.Text"),{},Microsoft::UI::Xaml::Controls::InfoBarSeverity::Success);Refresh();}else SetStatus(m_strings->GetString(L"Sessions.SaveFailed.Text"),{},Microsoft::UI::Xaml::Controls::InfoBarSeverity::Error);}
    ...
    void SessionsViewModel::ImportText(winrt::hstring const& text){if(!m_repo||!m_dispatcher)return;ImportTextAsync(text);}
    winrt::fire_and_forget SessionsViewModel::ImportTextAsync(winrt::hstring text)
    {
        auto life = get_strong();
        try
        {
            ...
            ::AstralChronicle::models::DiagnosticSession s;
            s.Filter = L"*"; s.Channels = L"System";
            bool active{};
            std::size_t start{};
            auto commit = [&]
            {
                if (!active || s.Name.empty()) return;
                s.Id = NewId() + L"-import";
                if (s.CreatedAt.empty()) s.CreatedAt = Now();
                if (repo->Upsert(s)) ++imported; else ++failed;
                s = {}; s.Filter = L"*"; s.Channels = L"System"; active = false;
            };
            ...
            if (failed != 0 || imported == 0)
                SetStatus(m_strings->GetString(L"Sessions.ImportFailed.Text"),{},Microsoft::UI::Xaml::Controls::InfoBarSeverity::Error);
            else
            {
                SetStatus(m_strings->GetString(L"Sessions.Imported.Text"),{},Microsoft::UI::Xaml::Controls::InfoBarSeverity::Success);
                Refresh();
            }
        }
        catch (...)
        {
            // Parsing, repository, and dispatcher failures must not terminate a fire-and-forget coroutine.
        }
    }
